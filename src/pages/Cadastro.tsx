...
export default function Cadastro() {
  ...
  async function submit() {
    ...
    setError("");
    try {
      const { session } = await signUp(cleanName, cleanEmail, senha);
        ...
        >
          {sent ? (
            <>
              ...
              <p className="auth-sub">Enviamos um link de confirmação para <strong>{email.trim().toLowerCase()}</strong>. Confirme para ativar sua conta e acessar o painel.</p>
              <Button variant="primary" onClick={() => nav("/login")} className="auth-submit" style={{ marginTop: 22 }}>
            ...
            <>
              ...
              <p className="auth-sub">Comece a organizar seus atendimentos em minutos.</p>
              ...
              <div className="auth-form">
                ...
                <Button variant="primary" onClick={submit} className="auth-submit" disabled={loading}>
                  {loading ? "Criando..." : "Criar conta"}{!loading && <ArrowRight size={18} />}
                </Button>

                <p className="auth-switch">
                  Já tem uma conta? <Link to="/login">Entrar <ArrowRight size={14} /></Link>
                </p>
              </div>
            </>
          )}
