        ...
        <Modal title="Novo questionário" sub="Monte uma vez e reutilize com todos os pacientes" onClose={() => setBuilder(false)} max={560}
          footer={<>
            ...
            <Button variant="primary" disabled={!name.trim()} onClick={() => {
              setList([{ id: Math.random().toString(36).slice(2), nome: name, categoria: "Consumo", perguntas: perguntas.filter((p) => p.t.trim()).length || 1, respostas: 0, atualizado: "17/06/2026", cor: "var(--sage)" }, ...list]);
              setBuilder(false); toast("Questionário criado");
            }}>Criar questionário</Button>
          </>}>
